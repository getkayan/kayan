...
| OAuth 2.0 and OIDC provider | Experimental | Authorization code uses PKCE; UserInfo and the `at_hash`/`c_hash` bindings are implemented; access tokens are signed and introspected through the key provider, so rotation applies to them; protocol audit persistence has an explicit failure callback; formal certification remains pending |
| SAML 2.0 | Experimental | Encrypted assertions are supported with RSA-OAEP key transport and AES-GCM content encryption; rsa-1_5 and CBC modes are refused. Metadata retrieval is injectable, bounded, and public-HTTPS-only by default; interoperability evidence remains pending |
