...
| SAML 2.0 | Experimental | Encrypted assertions are supported with RSA-OAEP key transport and AES-GCM content encryption; rsa-1_5 and CBC modes are refused. Single logout covers SP-initiated requests and verified inbound ones. Metadata retrieval is injectable, bounded, and public-HTTPS-only by default; interoperability evidence remains pending |
